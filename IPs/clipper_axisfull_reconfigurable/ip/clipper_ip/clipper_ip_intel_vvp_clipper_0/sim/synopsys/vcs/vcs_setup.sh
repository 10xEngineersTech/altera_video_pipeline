...
TOP_LEVEL_NAME="clipper_ip_intel_vvp_clipper_0"
QSYS_SIMDIR="./../../"
...
source $QSYS_SIMDIR/common/vcs_files.tcl
set files [dict merge $files [clipper_ip_intel_vvp_clipper_0::get_common_design_files "$QSYS_SIMDIR"]]
...
set QUARTUS_INSTALL_DIR [lindex $argv 2]
source $QSYS_SIMDIR/common/vcs_files.tcl
if {[catch {clipper_ip_intel_vvp_clipper_0::get_design_files "$QSYS_SIMDIR" "$QUARTUS_INSTALL_DIR"} errorMsg]} {
 puts "Catch error: $errorMsg"
 exit 1 
}
set files [dict create]
set files [dict merge $files [clipper_ip_intel_vvp_clipper_0::get_design_files "$QSYS_SIMDIR" "$QUARTUS_INSTALL_DIR"]]
...
run_vcs="vcs -lca -timescale=1ps/1ps -sverilog +verilog2001ext+.v $ELAB_OPTIONS $USER_DEFINED_ELAB_OPTIONS \
  ...
  -v $QUARTUS_SIM_LIB_DIR/cyclone10gx_atoms.v \
  -v $QUARTUS_SIM_LIB_DIR/synopsys/cyclone10gx_atoms_ncrypt.v \
  -v $QUARTUS_SIM_LIB_DIR/synopsys/cyclone10gx_hip_atoms_ncrypt.v \
  -v $QUARTUS_SIM_LIB_DIR/cyclone10gx_hip_atoms.v \
  -v $QUARTUS_SIM_LIB_DIR/synopsys/cyclone10gx_hssi_atoms_ncrypt.v \
  -v $QUARTUS_SIM_LIB_DIR/cyclone10gx_hssi_atoms.v \
  $common_design_files \
  $design_files \
  $USER_DEFINED_ELAB_OPTIONS_APPEND \
  -top $TOP_LEVEL_NAME
