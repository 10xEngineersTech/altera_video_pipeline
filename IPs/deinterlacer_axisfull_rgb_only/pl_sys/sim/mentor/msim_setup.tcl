...
# ----------------------------------------
# If pl_sys is one of several IP cores in your
...
# ----------------------------------------
# ACDS 25.1.1 125 linux 2026.04.30.12:11:56
...
if ![info exists TOP_LEVEL_NAME] { 
  set TOP_LEVEL_NAME "pl_sys.pl_sys"
}
...
set LD_LIBRARY_PATH [dict merge $LD_LIBRARY_PATH [dict get [pl_sys::get_env_variables $SIMULATOR_TOOL_BITNESS] "LD_LIBRARY_PATH"]]
if {[dict size $LD_LIBRARY_PATH] !=0 } {
  set LD_LIBRARY_PATH [subst [join [dict keys $LD_LIBRARY_PATH] ":"]]
  setenv LD_LIBRARY_PATH "$LD_LIBRARY_PATH"
}
append ELAB_OPTIONS [subst [pl_sys::get_elab_options $SIMULATOR_TOOL_BITNESS]]
append SIM_OPTIONS [subst [pl_sys::get_sim_options $SIMULATOR_TOOL_BITNESS]]
...
alias file_copy {
  ...
  set memory_files [concat $memory_files [pl_sys::get_memory_files "$QSYS_SIMDIR" "$QUARTUS_INSTALL_DIR"]]
  ...
  set normalized_src [pl_sys::normalize_path "$file"]
  set normalized_dest [pl_sys::normalize_path "$dest_file"]
...
set libraries [dict merge $libraries [pl_sys::get_dpi_libraries "$QSYS_SIMDIR"]]
...
set design_libraries [dict merge $design_libraries [pl_sys::get_design_libraries]]
...
alias com {
  ...
  set design_files [dict merge [pl_sys::get_common_design_files $USER_DEFINED_COMPILE_OPTIONS $USER_DEFINED_VERILOG_COMPILE_OPTIONS $USER_DEFINED_VHDL_COMPILE_OPTIONS "$QSYS_SIMDIR"]]
  set common_design_files [dict values $design_files]
  foreach file $common_design_files {
    eval $file
  }
  set design_files [list]
  set design_files [concat $design_files [pl_sys::get_design_files $USER_DEFINED_COMPILE_OPTIONS $USER_DEFINED_VERILOG_COMPILE_OPTIONS $USER_DEFINED_VHDL_COMPILE_OPTIONS "$QSYS_SIMDIR" "$QUARTUS_INSTALL_DIR"]]
