...
module pipeline_intel_vvp_pipeline2_0_altera_merlin_axi_slave_ni_19122_y6r6uii
#(
    // -----------------------------------------------
...
(
   ...
   endgenerate
    ...
    // pass through ace_lite signals
       ...
       if (ACE5_LITE_SUPPORT == 0) 
           ...
           assign write_cmd_snoop          = write_cp_data[PKT_SNOOP_H : PKT_SNOOP_L];   // 4 bits
       assign write_cmd_bar            = write_cp_data[PKT_BARRIER_H : PKT_BARRIER_L];
       assign write_cmd_unique         = write_cp_data[PKT_WUNIQUE];
       assign write_cmd_data_less      = write_cp_data[PKT_DATALESS];
       assign write_cmd_cache          = write_cp_data[PKT_CACHE_H : PKT_CACHE_L];
       
    //ACE5-Lite Signals 
    
    assign write_cmd_awatop        = write_cp_data[PKT_AWATOP_H      : PKT_AWATOP_L] ;
    assign write_cmd_awstashnid    = write_cp_data[PKT_AWSTASHNID_H  : PKT_AWSTASHNID_L] ;  
    assign write_cmd_awstashniden  = write_cp_data[PKT_AWSTASHNIDEN] ;      
    assign write_cmd_awstashlpid   = write_cp_data[PKT_AWSTASHLPID_H : PKT_AWSTASHLPID_L] ; 
    assign write_cmd_awstashlpiden = write_cp_data[PKT_AWSTASHLPIDEN] ; 
    assign write_cmd_awmmusecsid   = write_cp_data[PKT_MMUSECSID ] ;
    assign write_cmd_awmmusid      = write_cp_data[PKT_MMUSID_H    : PKT_MMUSID_L] ;
    ...
    generate
      if(ENABLE_OOO == 0)begin
        assign write_rsp_fifo_indata[ST_DATA_W]     = (data_less_txn) ? 1'h1 : write_cp_endofpacket;
        assign write_rsp_fifo_indata[ST_DATA_W-1:0] = write_cp_data;
      end
    endgenerate  
   ...
   endgenerate 
    ...
    assign internal_write_cp_endofburst =  (data_less_txn) ? write_cp_endofpacket  : (write_cmd_bytecount == minimum_bytecount);
    ...
    assign  write_cp_ready = (aw_w_ready && write_rsp_fifo_inready) || (address_taken && wready) || (data_taken && awready) ;
    assign  awvalid = write_cp_valid && !address_taken && write_rsp_fifo_inready;  
    assign  wvalid_tmp  = write_cp_valid && !data_taken && write_rsp_fifo_inready;    
    ...
    assign internal_write_to_fifo  = (data_less_txn) ? write_cp_startofpacket : internal_write_cp_endofburst ;
    assign write_rsp_fifo_invalid  = write_cp_ready &&  write_cp_valid && internal_write_to_fifo;
    ...
    // pass through ace_lite signals
       ...
       assign read_cmd_armmusecsid    = read_cp_data[PKT_MMUSECSID] ;
       assign read_cmd_armmusid       = read_cp_data[PKT_MMUSID_H : PKT_MMUSID_L] ;
...
// start of packet assertion should only happen at the start of burst and should not be asserted at the start of sub-burst
 ...
 generate 
   if (ENABLE_OOO == 0) begin
    ...
    pipeline_intel_vvp_pipeline2_0_altera_merlin_axi_slave_ni_altera_avalon_sc_fifo_19122_qekkyji #(
        .SYMBOLS_PER_BEAT    (1),
...
generate 
   if (ENABLE_OOO == 1) begin
    ...
    pipeline_intel_vvp_pipeline2_0_altera_merlin_axi_slave_ni_altera_avalon_st_pipeline_stage_19122_4ksnpba #(
      .SYMBOLS_PER_BEAT (1),
    ...
    pipeline_intel_vvp_pipeline2_0_altera_merlin_axi_slave_ni_altera_avalon_st_pipeline_stage_19122_ime5u4a #(
      .SYMBOLS_PER_BEAT (1),
...
generate
   ...
   if(ENABLE_OOO ==1 && REORDER_BUFFER == 0)begin
       ...
       pipeline_intel_vvp_pipeline2_0_altera_merlin_axi_slave_ni_altera_avalon_st_pipeline_stage_19122_di232tq #(
             .SYMBOLS_PER_BEAT (1),
