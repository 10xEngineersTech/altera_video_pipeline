...
source "$script_dir/pipeline_intel_vvp_pipeline2_0_emif_io96b_ddr4comp_400_pftbeyi_timing_parameters.tcl"
source "$script_dir/pipeline_intel_vvp_pipeline2_0_emif_io96b_ddr4comp_400_pftbeyi_timing_pins.tcl"
...
if { ! [ info exists pipeline_intel_vvp_pipeline2_0_emif_io96b_ddr4comp_400_pftbeyi_sdc_cache ] } {
   emif_initialize_ddr_db pipeline_intel_vvp_pipeline2_0_emif_io96b_ddr4comp_400_pftbeyi_emif_ddr_db var
   set pipeline_intel_vvp_pipeline2_0_emif_io96b_ddr4comp_400_pftbeyi_sdc_cache 1
} else {
   if { $debug } {
...
set instances [ array names pipeline_intel_vvp_pipeline2_0_emif_io96b_ddr4comp_400_pftbeyi_emif_ddr_db ]
foreach { inst } $instances {
   if { [ info exists pins ] } {
      unset pins
   }
   array set pins $pipeline_intel_vvp_pipeline2_0_emif_io96b_ddr4comp_400_pftbeyi_emif_ddr_db($inst)
   ...
   set pll_ncntr_clk [emif_get_or_add_generated_clock \
      ...
      set emifclks [get_clocks "${inst}_pll_ncntr"]
   ...
   foreach { vco_clock } $pins(pll_vco_clock) vco_base $pins(vco_base_node) {
      ...
      emif_get_or_add_generated_clock -target $vco_base \
         ...
         -divide_by 1 \
         -phase 0 
      set emifclks [add_to_collection $emifclks [get_clocks "${inst}_vco_base_${i_vco_clock}"]]
   ...
   foreach { vco_clock } $pins(pll_vco_periph_clock) vco_base $pins(vco_base_node) {
      set local_pll_vco_clk__p${i_vco_clock} [ emif_get_or_add_generated_clock \
         ...
         -divide_by 1 \
         -phase 0 ]
      set emifclks [add_to_collection $emifclks [get_clocks "${inst}_vco_clk_periph_${i_vco_clock}"]]
      set_clock_uncertainty -from ${inst}_vco_clk_periph_${i_vco_clock} -to ${inst}_vco_clk_periph_${i_vco_clock} -hold 0.020
      incr i_vco_clock
   }
   ...
   foreach { phy_clock } $pins(pll_phy_clock) phy_reg $pins(pll_phy_reg) {
      ...
      emif_get_or_add_generated_clock -target $phy_reg \
         ...
         -divide_by $divide_by \
         -phase $phase
      set emifclks [add_to_collection $emifclks [get_clocks "${inst}_c0_cntr_${i_phy_clock}"]]
      set local_phy_clk_${i_phy_clock} [ emif_get_or_add_generated_clock \
         ...
         -divide_by 1 \
         -phase $phase ]
      set emifclks [add_to_collection $emifclks [get_clocks "${inst}_phy_clk_${i_phy_clock}"]]
   ...
   foreach { phy_clock_l } $pins(pll_phy_clock_sync) phy_reg_s $pins(pll_phy_reg_sync) {
      ...
      emif_get_or_add_generated_clock -target $phy_reg_s \
         ...
         -divide_by $divide_by \
         -phase $phase
      set emifclks [add_to_collection $emifclks [get_clocks "${inst}_c1_cntr_${i_phy_clock_l}"]]
      set local_phy_clk_sync_${i_phy_clock_l} [ emif_get_or_add_generated_clock \
         ...
         -divide_by 1 \
         -phase $phase ]
      set emifclks [add_to_collection $emifclks [get_clocks "${inst}_phy_clk_sync_${i_phy_clock_l}"]]
   ...
   if {!($var(PHY_NOC_EN) || $var(MEM_CLK_ASYNC) || ($var(LOCKSTEP_ROLE) ne "OFF" && [regexp "1\$" $inst]))} {
      ...
      set usr_clock [ emif_get_or_add_generated_clock \
         ...
         -phase $phase ]
      set emifclks [add_to_collection $emifclks [get_clocks "${inst}_usr_clk"]]
   }
   ...
   foreach {dqs_t} $pins(dqs_t) {
      ...
      set emifclks [add_to_collection $emifclks [get_clocks "${inst}_${dqs_t}_in"]]
...
#set rclk_idx 0
   ...
   foreach {fa_phyclk} $pins(phyclk_div) {
      emif_get_or_add_generated_clock \
         ...
         -divide_by $fa_div \
         -phase 0
      set emifclks [add_to_collection $emifclks [get_clocks "${inst}_fa_[lindex $fa_phyclk 1]_[lindex $fa_phyclk 2]_[lindex $fa_phyclk 3]"]]
   }
   ...
   foreach t $pins(dqs_t) n $pins(dqsn_nff) {
      if {$var(MEM_DQ_WIDTH) >= 64 && $n == "" && \
            (($var(MEM_TECHNOLOGY) == "DDR5" && [llength $pins(dqs_t)] > 5) || \
             ($var(MEM_TECHNOLOGY) == "DDR4" && [llength $pins(dqs_t)] > 5)) } {   
         continue;
      }
   ...
   if {!$var(PHY_NOC_EN) && $var(LOCKSTEP_ROLE)=="OFF" && $var(MEM_CHANNEL_DATA_DQ_WIDTH) != 40} {
      regexp {(.*)_(\d)} $inst "" baseinst instidx
      set_min_delay -to [get_registers ${baseinst}*arch_${instidx}*hmc_core_reg] -0.170

      if {$var(MEM_CLK_ASYNC)} {
         set_clock_groups -exclusive -group $emifclks
      } else {
         set_clock_groups -exclusive -group "${inst}_usr_clk" -group [get_clocks "${inst}_phy_clk_*"] 
      }
   }
   
   # LOCKSTEP
   if {$var(LOCKSTEP_ROLE) ne "OFF" && [regexp "0\$" $inst]} {
	set ls_inst [regsub "_0\$" $inst {}]
        if ${sta_flow} {
            set_min_delay -20 -from [get_keepers ${ls_inst}|lockstep.ls_sideband_adaptor|*|*_mcp*]
            set_max_delay  20 -from [get_keepers ${ls_inst}|lockstep.ls_sideband_adaptor|*|*_mcp*]
        ...
        set_false_path -from [get_keepers ${ls_inst}|*hmc_*fa_reg] -to [get_keepers ${ls_inst}|*hmc_periph_reg]
        set_false_path -from [get_keepers ${ls_inst}|*lane_*fa_reg] -to [get_keepers ${ls_inst}|*hmc_periph_reg]
        set_false_path -from [get_keepers ${ls_inst}|*ssm_*fa_reg] -to [get_keepers ${ls_inst}|*hmc_periph_reg]
        if {[get_collection_size [get_keepers -nowarn ${ls_inst}|*seq~phy_a_reg]] > 0} {
           set_false_path -from [get_keepers ${ls_inst}|*ssm_*fa_reg] -to [get_keepers ${ls_inst}|*seq~phy_a_reg]
        }
        if {[get_collection_size [get_keepers -nowarn ${ls_inst}|*seq~phy_b_reg]] > 0} {
           set_false_path -from [get_keepers ${ls_inst}|*ssm_*fa_reg] -to [get_keepers ${ls_inst}|*seq~phy_b_reg]
        }
        set_false_path -from [get_keepers ${ls_inst}|*hmc_periph_reg] -to [get_keepers ${ls_inst}|*hmc_*fa_reg]
        set_false_path -from [get_keepers ${ls_inst}|*hmc_periph_reg] -to [get_keepers ${ls_inst}|*lane_*fa_reg]
        set_false_path -from [get_keepers ${ls_inst}|*hmc_periph_reg] -to [get_keepers ${ls_inst}|*ssm_*fa_reg]                
        set_false_path -through ${ls_inst}|arch_emif_ls_0.arch0_1ch_per_io.arch_0|wrapper_cpa|cpa|o_core_clk_out -to [get_keepers ${ls_inst}|*hmc_periph_reg]
    } elseif {$var(MEM_CHANNEL_DATA_DQ_WIDTH) == 40} {
        set ls_inst [regsub "_0\$" $inst {}]
        set_multicycle_path -from [get_keepers ${ls_inst}|*hmc_*fa_reg] -to [get_keepers ${ls_inst}|*hmc_periph_reg] -end -setup 2
        set_multicycle_path -from [get_keepers ${ls_inst}|*hmc_*fa_reg] -to [get_keepers ${ls_inst}|*hmc_periph_reg] -hold 1
        set_multicycle_path -from [get_keepers ${ls_inst}|*lane_*fa_reg] -to [get_keepers ${ls_inst}|*hmc_periph_reg] -end -setup 2
        set_multicycle_path -from [get_keepers ${ls_inst}|*lane_*fa_reg] -to [get_keepers ${ls_inst}|*hmc_periph_reg] -hold 1
        set_multicycle_path -from [get_keepers ${ls_inst}|*hmc_periph_reg] -to [get_keepers ${ls_inst}|*hmc_*fa_reg] -start -hold 1
        set_multicycle_path -from [get_keepers ${ls_inst}|*hmc_periph_reg] -to [get_keepers ${ls_inst}|*lane_*fa_reg] -start -hold 1
        set_false_path -through ${ls_inst}|arch_emif_0.arch0_1ch_per_io.arch_0|wrapper_cpa|cpa|o_core_clk_out -to [get_keepers ${ls_inst}|*hmc_periph_reg]
    }
   ...
   if {[llength $pins(rclk)] > 0} {
      ...
      if {$var(MEM_TECHNOLOGY) == "LPDDR5"} {
         foreach rdqs $pins(rclk) {
            if {[regexp "rdqs_t" $rdqs]} {
               set_output_delay -clock $pins(ref_clock_name) 0 $rdqs -add
            }
         }
      }
   }
   ...
   if {$var(MEM_TECHNOLOGY) == "LPDDR5"} {
      disable_min_pulse_width [get_clocks "$inst*rdqs_t*nff"]
   }
}
...
if {  $var(LOCKSTEP_ROLE) ne "OFF" } {
    add_ddr_report_command "source [list [file join [file dirname [info script]] pipeline_intel_vvp_pipeline2_0_emif_io96b_ddr4comp_400_pftbeyi_report_timing.tcl]]"
}
