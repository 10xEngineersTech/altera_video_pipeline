...
# ----------------------------------------
# If pipeline is one of several IP cores in your
...
# ----------------------------------------
# ACDS 25.1.1 125 linux 2026.05.29.16:33:38
...
if ![info exists TOP_LEVEL_NAME] { 
  set TOP_LEVEL_NAME "pipeline.pipeline"
}
...
set LD_LIBRARY_PATH [dict merge $LD_LIBRARY_PATH [dict get [pipeline::get_env_variables $SIMULATOR_TOOL_BITNESS] "LD_LIBRARY_PATH"]]
if {[dict size $LD_LIBRARY_PATH] !=0 } {
  set LD_LIBRARY_PATH [subst [join [dict keys $LD_LIBRARY_PATH] ":"]]
  setenv LD_LIBRARY_PATH "$LD_LIBRARY_PATH"
}
append ELAB_OPTIONS [subst [pipeline::get_elab_options $SIMULATOR_TOOL_BITNESS]]
append SIM_OPTIONS [subst [pipeline::get_sim_options $SIMULATOR_TOOL_BITNESS]]
...
alias file_copy {
  ...
  set memory_files [concat $memory_files [pipeline::get_memory_files "$QSYS_SIMDIR" "$QUARTUS_INSTALL_DIR"]]
  ...
  set normalized_src [pipeline::normalize_path "$file"]
  set normalized_dest [pipeline::normalize_path "$dest_file"]
...
set libraries [dict merge $libraries [pipeline::get_dpi_libraries "$QSYS_SIMDIR"]]
...
set design_libraries [dict merge $design_libraries [pipeline::get_design_libraries]]
...
alias com {
  ...
  set design_files [dict merge [pipeline::get_common_design_files $USER_DEFINED_COMPILE_OPTIONS $USER_DEFINED_VERILOG_COMPILE_OPTIONS $USER_DEFINED_VHDL_COMPILE_OPTIONS "$QSYS_SIMDIR"]]
  set common_design_files [dict values $design_files]
  foreach file $common_design_files {
    eval $file
  }
  set design_files [list]
  set design_files [concat $design_files [pipeline::get_design_files $USER_DEFINED_COMPILE_OPTIONS $USER_DEFINED_VERILOG_COMPILE_OPTIONS $USER_DEFINED_VHDL_COMPILE_OPTIONS "$QSYS_SIMDIR" "$QUARTUS_INSTALL_DIR"]]
