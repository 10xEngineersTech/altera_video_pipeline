...
//     CSC registers are being written. First frame uses default coefficients.
...
//
// TPG port width parameters:
//   TPG_TDATA_W = 24 for 444/420 (NUMBER_OF_COLOR_PLANES=3)
//   TPG_TDATA_W = 16 for 422     (NUMBER_OF_COLOR_PLANES=2)
//   TPG_TUSER_W = 3  for 444/420
//   TPG_TUSER_W = 2  for 422
//   Must match regenerated pipeline.v TPG port widths.
// =============================================================================
...
module top #(
    ...
    // 444/420 (NUMBER_OF_COLOR_PLANES=3): TDATA=24, TUSER=3
    // 422     (NUMBER_OF_COLOR_PLANES=2): TDATA=16, TUSER=2
    parameter        TPG_TDATA_W     = 24,
    parameter        TPG_TUSER_W     = 3
)(
    input  wire        clk,
    input  wire        reset,
    ...
    localparam [11:0] CLIP_R_OR_W     = CLIP_BASE | 12'h150;  // 0x750
    localparam [11:0] CLIP_B_OR_H     = CLIP_BASE | 12'h154;  // 0x754

    // --- SCALER base = 0x800 (Lite mode) ---
    localparam [11:0] SCL_BASE        = 12'h800;
    localparam [11:0] SCL_IN_WIDTH    = SCL_BASE | 12'h120;  // 0x920
    localparam [11:0] SCL_IN_HEIGHT   = SCL_BASE | 12'h124;  // 0x924
    localparam [11:0] SCL_OUT_WIDTH   = SCL_BASE | 12'h148;  // 0x948
    localparam [11:0] SCL_OUT_HEIGHT  = SCL_BASE | 12'h14C;  // 0x94C
    ...
    // =========================================================================
    // CSC coefficient ROMs (Q10.21 format)
    // =========================================================================
    ...
    always @(posedge clk or posedge reset) begin
        if (reset) begin
            ...
            case (current_state)
                ...
                // ==============================================================
                // TPG MM configuration (own Avalon port)
                // ==============================================================
                ST_TPG_CTRL_1: begin
                    tpg_write <= 1'b1;
                ...
                // ==============================================================
                // CLIPPER: LEFT(0) TOP(1) RIGHT(2) BOTTOM(3) COMMIT(4)
                // ==============================================================
                ...
                // ==============================================================
                // SCALER: IN_W(0) IN_H(1) OUT_W(2) OUT_H(3)
                // ==============================================================
                ST_CONFIG_SCL: begin
                    bridge_write <= 1'b1;
                    if (bridge_write && !bridge_wait) begin
                ...
                // ==============================================================
                // CSC: A0 B0 C0 A1 B1 C1 A2 B2 C2 S0 S1 S2 OUT_CS COMMIT
                // ==============================================================
                ST_CONFIG_CSC: begin
                    bridge_write <= 1'b1;
                    if (bridge_write && !bridge_wait) begin
                ...
                // ==============================================================
                // PC1: WIDTH(0) HEIGHT(1) INTERLACE(2) COLORSPACE(3) SUBSAMP(4) CTRL(5)
                // ==============================================================
                ST_CONFIG_PC1: begin
                    pc1_write <= 1'b1;
    ...
    // =========================================================================
    // Video wires ? parametric TPG width
    // TPG_TDATA_W=24/TPG_TUSER_W=3 for 444/420
