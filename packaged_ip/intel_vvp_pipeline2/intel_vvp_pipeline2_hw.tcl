...
set_parameter_property TOPOLOGY ALLOWED_RANGES       {FULL SCALER_ONLY CSC_ONLY CRS_CSC CLIP_SCL DIL_ONLY}
...
# ============================================================================
# GUI LAYOUT ? tabs matching the mockup
# ============================================================================
 
# ?? Diagram tab (static HTML overview) ??????????????????????????????????????
...
<tr><td><b>Full pipeline</b></td><td>Input ? DIL &rarr; CRS &rarr; CSC &rarr; Clipper &rarr; PC0 &rarr; Scaler ? Output</td><td>DIL, CRS, CSC, Clipper, Scaler</td></tr>
<tr><td><b>Scaler only</b></td><td>Input ? Scaler ? Output</td><td>Scaler</td></tr>
<tr><td><b>CSC only</b></td><td>Input ? CSC ? Output</td><td>CSC</td></tr>
<tr><td><b>CRS + CSC</b></td><td>Input ? CRS &rarr; CSC ? Output</td><td>CRS, CSC</td></tr>
<tr><td><b>Clipper + Scaler</b></td><td>Input ? Clipper &rarr; PC0 &rarr; Scaler ? Output</td><td>Clipper, Scaler</td></tr>
<tr><td><b>Deinterlacer only</b></td><td>Input ? DIL ? Output (separate ports)</td><td>DIL</td></tr>
</table><br>
...
add_display_item crs_tab crs_horiz group "Horizontal resampling"
...
add_display_item crs_tab crs_vert group "Vertical resampling"
...
add_display_item crs_tab crs_ctrl group "Control"
...
proc compose {} {
    ...
    # ?? PC0 ????????????????????????????????????????????????????????????????
    # PC0 converts from Altera SV Full (internal) to user-selected output protocol
    # Auto-inserted whenever protocol conversion is needed at output boundary
    if {$do_pc0} {
        set pconv_in_mode  "INTERNAL"
        set pconv_out_mode "EXTERNAL"
        ...
        set_instance_parameter_value intel_vvp_protocol_conv_0 INPUT_MODE             $pconv_in_mode
        set_instance_parameter_value intel_vvp_protocol_conv_0 OUTPUT_MODE            $pconv_out_mode
    ...
    # DIL separate video ports (always separate ? not part of main chain input)
    if {$do_dil} {
        ...
        if {!$do_crs} {
            # DIL-only mode: export DIL video out as main output
            ...
            # FULL mode: DIL feeds CRS (connected above); still expose DIL in port
            add_interface s_axis_video_in axi4stream end
            set_interface_property s_axis_video_in EXPORT_OF intel_vvp_dil_0.axi4s_vid_in
        }
    }
